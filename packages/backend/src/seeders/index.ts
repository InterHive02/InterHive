...
async function bootstrap() {
  ...
  try {
    ...
    const internProfileModel = app.get(getModelToken('InternProfile'));
    const internReadinessModel = app.get(getModelToken('InternReadiness'));
    const companyModel = app.get(getModelToken('Company'));
    const companyRequirementModel = app.get(getModelToken('CompanyRequirement'));
    const dummyAccounts = [
      {
        firstName: 'Alex',
        lastName: 'Admin',
    ...
    for (const account of dummyAccounts) {
      ...
      // Seed Intern Profile & Readiness for Intern Role
      if (account.role === UserRole.INTERN) {
      ...
      // Seed Company Profile for Company Role
      if (account.role === UserRole.COMPANY) {
        ...
        if (!company) {
          ...
          // Seed a sample requirement for company
